...
export class PolicyEngine {
  ...
  async getBNBPrice(): Promise<number> {
    ...
    if (
      this.bnbPriceCache &&
      now - this.bnbPriceCache.timestamp < this.PRICE_CACHE_MS
    ) {
  ...
  async checkTx(
    ...
    if (
      ...
      if (!this.config.allowedTokens.includes(options.token.toLowerCase())) {
        return {
          allowed: false,
          reason: `Token ${options.token} is not in the allowed tokens list.`,
        };
      }
    }
    ...
    if (
      this.config.allowedContracts.length > 0 &&
      !this.config.allowedContracts.includes(to.toLowerCase())
    ) {
