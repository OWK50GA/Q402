...
import { base64EncodeJson, settlePayment, typify } from "@/lib/utils";
import { createWalletClient, stringify, type WalletClient } from "viem";
...
function create402Response(
  config: Q402Config,
  endpoint: Q402Config["endpoints"][0],
  amount: string,
) {
  ...
  const paymentDetails = {
    ...
    token: endpoint.token as `0x${string}`,
    amount,
    to: config.recipientAddress as `0x${string}`,
    implementationContract: config.implementationContract as `0x${string}`,
    witness: {
      domain: {
        name: "q402",
        version: "1",
    ...
    authorization: {
      chainId: networkConfig.chainId,
      address: config.implementationContract as `0x${string}`,
      nonce: 0,
    },
  };
  ...
  return async (req: NextRequest) => {
    try {
      ...
      if (!paymentHeader) {
        ...
        const response = create402Response(config, endpoint, txPayload.amount);

        console.log(typify(response));

        // const intermediateJSONStringifiedResponse = stringify(response, (_k, v) => {
        //   if (typeof v === 'bigint') return v.toString()
        //     else return v
        // })
        // const parsedResponse = parse(intermediateJSONStringifiedResponse)
        // response.accepts[0].amount = txPayload.amount;
        // @ts-ignore
        // response.accepts[0].witness.message.amount = txPayload.amount;
      ...
      const tokenMismatch =
        endpoint.token !== "any" &&
        payload.paymentDetails.token !== endpoint.token;
      const amountMismatch =
        endpoint.amount !== "any" &&
        String(payload.paymentDetails.amount) !== String(endpoint.amount);
      const toMismatch =
        !config.allowAnyRecipient &&
        payload.paymentDetails.to !== config.recipientAddress;
      ...
      if (tokenMismatch || amountMismatch || toMismatch) {
        return NextResponse.json(
          { error: "Payment details do not match endpoint configuration" },
          { status: 402 },
        );
      }

      const deadline = Number(
        payload.paymentDetails.witness.message.deadline || 0,
      );
      ...
      if (!verificationResult.isValid) {
        ...
        if (
          config.devMode &&
          verificationResult.invalidReason === "invalid_authorization"
        ) {
          console.log(
            "Development mode: accepting payload despite missing signature fields",
          );
        } else {
          return NextResponse.json(
            {
      ...
      const paymentInfo: Q402PaymentInfo = {
        ...
        amount:
          "amount" in payload.paymentDetails
            ? payload.paymentDetails.amount
            : undefined,
        token:
          "token" in payload.paymentDetails
            ? payload.paymentDetails.token
            : undefined,
      };
      ...
      if (config.autoSettle !== false) {
        try {
          ...
          if (settlementResult.success) {
            ...
            settlementHeaders[X_PAYMENT_RESPONSE_HEADER] =
              base64EncodeJson(executionResponse);
          } else {
