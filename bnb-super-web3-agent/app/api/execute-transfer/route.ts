...
if (!implementationContract)
  throw new Error("Q402_IMPLEMENTATION_CONTRACT required");
...
const sponsorPrivateKey = sponsorPrivateKeyRaw.startsWith("0x")
  ? sponsorPrivateKeyRaw
  : `0x${sponsorPrivateKeyRaw}`;

// Validate length/format: must be a 32-byte hex string (0x + 64 hex chars)
if (!/^0x[0-9a-fA-F]{64}$/.test(sponsorPrivateKey)) {
  throw new Error(
    "SPONSOR_PRIVATE_KEY must be a 32-byte hex string (0x followed by 64 hex characters)",
  );
}
