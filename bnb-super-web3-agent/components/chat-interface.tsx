...
export default function ChatInterface() {
  ...
  const [previewDrawerData, setPreviewDrawerData] =
  ...
  const determineResponseType = (input: string): MessageCategory => {
    ...
    if (
      lowerInput.includes("pragma solidity") ||
  ...
  const generateResponse = async (userMessage: string) => {
    ...
    try {
        ...
        const onChunk = (chunk: string) => {
          ...
          if (endIdx !== -1) {
            ...
            try {
              ...
              parsed.paymentDetails.witness.message.owner =
              ...
              (async () => {
                try {
                  const res = await fetch("/api/execute-transfer", {
                    ...
                    body: JSON.stringify({
                      txPayload: parsed.paymentDetails ?? null,
                    }),
                  });
                  ...
                  if (res.status === 402) {
                    ...
                    const paymentDetails = selectPaymentDetails(
                      first402Response,
                      {
                    ...
                    // paymentDetails?.witness.message.amount ? amount;
                    ...
                    console.log(
                      "Payment details typified: ",
                      typify(paymentDetails),
                    );
                    console.log(
                      "Typify stable Stringify payment details: ",
                      stableStringify(typify(paymentDetails)),
                    );
                    try {
                      const paymentHeader = await createPaymentHeaderLocal(
                        // @ts-ignore
                        activeAccount,
                        paymentDetails,
                      );
                      ...
                      prev.map((m) =>
                        m.id === agentId
                          ? {
                              ...
                              content:
                                (m.content || "") +
                                "\n\n[Payload accepted — execute-transfer started]",
                            }
                          : m,
                      ),
                    );
                    ...
                    prev.map((m) =>
                      m.id === agentId
                        ? {
                            ...
                            content:
                              (m.content || "") +
                              "\n\n[Network error while starting transfer]",
                          }
                        : m,
                    ),
                  );
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === agentId ? { ...m, content: `Error: ${msg}` } : m,
        ),
      );
    } finally {
  ...
  return (
    ...
    >
        ...
        <div className="w-full flex items-center justify-between px-2">
          ...
          <div className="text-center">
            <h1 className="text-base font-semibold text-gray-900">
              Quack × ChainGPT Agent
            </h1>
            <p className="text-xs text-gray-500">
              Environment:{" "}
              {network === "bscTestnet" ? "BNB Testnet" : "BNB Mainnet"}
            </p>
          </div>
      ...
      <div
        ...
        className="flex-grow pb-40 pt-16 px-4 overflow-y-auto"
      >
          ...
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-[60vh] text-center">
              ...
              <p className="text-gray-500 max-w-md">
                Research tokens, audit contracts, or execute transactions with
                ...
                <div className="w-full max-w-[90%]">
                  ...
                  {message.researchData && (
                    <ResearchCard
                      ...
                      onAction={(action) =>
                        handleCardAction(action, message.id)
                      }
                    />
                  )}
                  ...
                  {message.actionData && (
                    <ActionCard
                      ...
                      onPreview={() =>
                        setPreviewDrawerData(message.actionData!)
                      }
                      onExecute={() => {
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            {detectedMode && (
              <div className="flex items-center gap-1.5 px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium animate-in fade-in slide-in-from-left">
                ...
                <span>Detected: {detectedMode}</span>
              </div>
            )}

            {(
              ["research", "audit", "action", "generate"] as MessageCategory[]
            ).map((mode) => (
              <button
                key={mode}
                type="button"
