...
export function ActionCard({ data, onPreview, onExecute }: ActionCardProps) {
  const isBlocked =
    data.isDenied ||
    !data.isAllowed ||
    data.spendCapUsage.amount > data.spendCapUsage.total;
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min((data.spendCapUsage.amount / data.spendCapUsage.total) * 100, 100)}%`,
              }}
            />
          </div>
          <p className="text-xs text-gray-500 mt-1">
            {data.spendCapUsage.remaining} remaining after this action
          </p>
        </div>
